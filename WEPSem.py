...
class WEPSem(Model):
	...
	def word_embedding_LLK_regul(self, ids, embed):
		...
		emb_log_sig = tf.clip_by_value(emb_log_sig, clip_value_min= -13.0, clip_value_max=5.0) ### clip-clamp
	...
	def add_placeholders(self):
		...
		self.adj_mat   		= tf.compat.v1.placeholder(tf.bool,      	shape=[self.num_labels, self.p.batch_size, None, None], name='adj_ind')
		self.num_words		= tf.compat.v1.placeholder(tf.float32,   	shape=[self.p.batch_size],         			name='num_words')
		...
		for layer in range(num_layers):
			...
			with tf.name_scope('%s-%d' % (name,layer)):
				...
				with tf.compat.v1.variable_scope('Loop-name-%s_layer-%d' % (name, layer)) as scope:
					w_loop   = tf.compat.v1.get_variable('w_loop',  initializer=tf.eye(in_dim), trainable=False, regularizer=self.regularizer)
				...
				for lbl in range(max_labels):
					...
					with tf.compat.v1.variable_scope('label-%d_name-%s_layer-%d' % (lbl, name, layer)) as scope:
						w_in   = tf.compat.v1.get_variable('w_in',  	initializer=tf.eye(in_dim), 	trainable=True,  regularizer=self.regularizer)
						w_out  = tf.compat.v1.get_variable('w_out', 	initializer=tf.eye(in_dim), 	trainable=True,  regularizer=self.regularizer)
						b_in   = tf.compat.v1.get_variable('b_in',   	[1,      gcn_dim],		trainable=True,  initializer=tf.constant_initializer(0.0),		regularizer=self.regularizer)
						b_out  = tf.compat.v1.get_variable('b_out',  	[1,      gcn_dim],		trainable=True,  initializer=tf.constant_initializer(0.0),		regularizer=self.regularizer)

						if w_gating:
							w_gin   = tf.compat.v1.get_variable('w_gin',  [in_dim, 1], 	initializer=tf.compat.v1.keras.initializers.VarianceScaling(scale=0.10, mode="fan_avg", distribution="uniform"),	regularizer=self.regularizer)
							b_gin   = tf.compat.v1.get_variable('b_gin',  [1], 		initializer=tf.constant_initializer(0.0),		regularizer=self.regularizer)
							w_gout  = tf.compat.v1.get_variable('w_gout', [in_dim, 1], 	initializer=tf.compat.v1.keras.initializers.VarianceScaling(scale=0.10, mode="fan_avg", distribution="uniform"), regularizer=self.regularizer)
							b_gout  = tf.compat.v1.get_variable('b_gout', [1], 		initializer=tf.constant_initializer(0.0),		regularizer=self.regularizer)
	...
	def add_model(self):
		...
		with tf.compat.v1.variable_scope('Embed_mat'):
			...
			_wrd_embed   	    = tf.compat.v1.get_variable('embed_matrix', initializer=embed_init, trainable=True, regularizer=self.regularizer)
	...
	def __init__(self, params):
		...
		self.orig_loss = self.add_loss_op(nn_out)
		self.loss = self.orig_loss + self.regul
	...
	def checkpoint(self, loss, epoch, sess):
		...
		print(results)
		self.stats.append([results['SimLex999'],  results['RW'], results['WS353'], results['WS353S'], results['WS353R'], results['AP'], results['MSR']])
		mdic = {"stats": np.asarray(self.stats, dtype=np.float32)}
		savemat("stats_WEPSem_llk_regul_gauss_2L.mat", mdic)
	...
	def run_epoch(self, sess, epoch, shuffle=True):
		...
		for step, batch in enumerate(self.getBatches(shuffle)):
			...
			loss, orig_loss, regul, _= sess.run([self.loss, self.orig_loss, self.regul, self.train_op], feed_dict=feed)
			losses.append(loss)
			orig_losses.append(orig_loss)
			reguls.append(regul)

			if (step+1) % 10 == 0:
				self.logger.info('E:{} (Sents: {}/{} [{}]): Loss=\t{:.6} orig_loss=\t{:.6} regul=\t{:.6} name=\t{} best_int_avg=\t{:.5}'.format(epoch, self.sent_num, self.p.total_sents, round(self.sent_num/self.p.total_sents * 100 , 1),
																								  np.mean(losses),  np.mean(orig_losses),  np.mean(reguls),  self.p.name, self.best_int_avg))
				en = time.time()
				if (en-st) >= 3600:
...
if __name__== "__main__":
	...
	parser.add_argument('-name',     dest="name",           default='WEPSem_llk_regu_gauss_2L',  	help='Name of the run')
	...
	parser.add_argument('-alpha',    dest="alpha",          default= 0.05,     type=float,   help='llk regul')
